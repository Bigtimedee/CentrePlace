...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-60 bg-slate-900 border-r border-slate-800 flex flex-col transition-transform duration-200 ease-in-out",
        ...
        {/* User button — hidden on mobile (shown in MobileHeader) */}
        <div className="h-16 hidden lg:flex items-center px-6 border-t border-slate-800">
          <UserButton />
        </div>
      </aside>
    </>
