  ...
  return (
    <div>
      ...
      <div className="mt-8 space-y-6">
        ...
        <Card>
          <CardBody>
            <p className="text-xs text-[#7A6B82] mb-4">
        ...
        {/* Error state */}
        {error && (
          <div className="rounded-xl border border-[#4A3257] bg-[#2D1B3D] px-6 py-10 text-center text-sm text-[#7A6B82]">
            {isProfileMissing ? (
              <>
                <p className="mb-3">Complete your profile to run scenario comparisons.</p>
                <Link
                  href="/profile"
                  className="inline-block rounded-lg bg-[#C8A45A] px-4 py-2 text-xs font-medium text-[#1A0F28] hover:bg-[#D4A574] transition-colors"
                >
                  Go to Profile →
                </Link>
              </>
        ...
        {/* Loading state */}
        {isLoading && !error && (
          <div className="flex items-center justify-center py-16 text-[#7A6B82] text-sm">
            <svg className="animate-spin h-5 w-5 mr-3 text-[#C8A45A]" fill="none" viewBox="0 0 24 24">
